...
### T-341 단일 책임 저장소 분리
- 저장소를 앱 전용 경로(`~/Library/Application Support/AIModelTalk/AIModelTalk.store`)로 분리. 기존 SwiftData 기본값(공유 `default.store`)은 타 앱과 공유 → 요청한 `ChatSessionEntity` 테이블이 없음에도 쿼리가 오류 없이 **0행** 반환 → 세션 저장·로드가 조용히 실패, 실행할 때마다 빈 화면이 되는 원인
- 원인 규명: 같은 `default.store`를 여는 다른 앱(`F6FF624B` 테이블) 존재 확인. AIModelTalk가 그 DB를 열면 INSERT는 타 앱 모델로 처리되어 재실행 시 로드 불가
- 판별 근거: 하드디스크 삭제가 아니라 공유 저장소 충돌로 확정 — 검색 인덱스에 4,958 문장·3,928 세션·마지막 업데이트 2026-09-05 16:13 잔존 (별도 파일이라 온전)
- 실패 무음화 제거: 세션 로드 실패 `E-MAC-STR-1002`, 저장 실패 `E-MAC-STR-1003` 로그 의무화
- 세션 insert 시에만 저장되던 모드·모드별 선택 모델(이미지/코딩/오디오) 영속, 메시지 followUps·toolRuns diff-upsert 갱신 추가
- 회귀 테스트 `PersistenceRegressionTestsV40` 3종 추가·통과 (전용 URL / 모드 라운드트립 / followUps 라운드트립)

### 기술적 영향
- `codesign` 서명 없이 also build. `default.store` 공유 충돌은 v0.3.3의 저장 방식(단순 DB 파일)과 조합되어 발생한 회귀 — 이번 저장소 분리로 원천 차단
...
### T-337 Follow up 후속질문
- **의미**: 이해하신 대로 — 모델이 다음 질문 3개를 만들어주고 클릭 즉시 전송
- **생성**: 응답 완료 후 보조 모델 1회 호출 (채팅 모드 텍스트 응답만), 실패 시 섹션 숨김. `ChatMessage.followUps` + Core Data 영속 (재생성 없음)
- **UI**: 말풍선 액션 행 아래 섹션 — 구분선 행 + hover `→` + 클릭 즉시 전송

### T-338 질문 목차
- "북마크" 대신 "질문 목차" 명명 — 채팅 툴바 버튼 → 내 질문 목록 팝오버 → 탭하면 점프 (검색 점프와 동일 인프라, 신규 스크롤 코드 없음)

### T-337b Follow up 우측 인덴트
- 전체 폭 펼침에서 응답 하단 우측 블록으로 변경 (`HStack Spacer 140` 인덴트) — 화살표가 블록 우변에 소속돼 깨짐 해소

### T-337c Follow up 아랫줄 이동
- 섹션이 푸터 HStack 안에 들어가 같은 줄에 찌그러지던 버그 수정 — VStack 직속으로 이동해 타임스탬프·복사 행 아랫줄 전체 차지

### T-337d Follow up 한글화
- 헤더 `"Follow up"` → `"후속 질문"`, 생성 프롬프트에 "반드시 한국어(존댓말)" 강화 (프롬프트 자체는 정상이었음)

### T-339 메시지 되돌리기
- 사용자 메시지 우클릭 → "이 지점부터 되돌리기" (파괴적 → 확인 다이얼로그) — 선택 메시지 **포함** 이후 전부 삭제 + 잘라낸 메시지를 입력창에 프리필 (**전송 없음**, 고쳐서 다시 전송)
- 기억 회수: 삭제 구간과 겹치는 자동·미핀 기억만 제거 (포함관계 OR 메시지 단위 bigram 0.25, 수동·핀 보호, 출처 없어 최선노력)
- 검색 인덱스는 saveSession 재구축으로 자동 동기화, 하단으로 스크롤

### T-340 사용자 말풍선 2줄
- 1행 입력 대화 / 2행 `보낸 시간 · 분기 · 되돌리기` 텍스트 버튼 행 (우측 정렬) — 우클릭 메뉴는 유지
